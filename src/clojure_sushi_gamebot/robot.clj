;;;
;;; This is a clone of Caleb M Peterson's awtbot
...
;;
(defmacro with-robot
  "Performs a series of actions with a robot"
  [robot & forms]
  (let [grobot (gensym)]
    `(let [~grobot ~robot]
       ~@(map (fn [f]
...
(defn chord-keys
  "Chord a series of keys. The keys are pressed in order
...
(defn scroll
  "Perform a scroll whell operation"
  [robot notches]
